...
First note that for any $\mathbf{x}\in V$, $\left< \mathbf{0}, \mathbf{x} \right>=0$, so that $\mathbf{0}\in V^{\perp}$.\\

Now, by the bilinearity of the inner product, $\left< \mathbf{a} + \mathbf{b}, x \right> = \left< \mathbf{a}, \mathbf{x} \right> + \left< \mathbf{b}, \mathbf{x} \right> = 0$ and $\left< r\mathbf{a}, \mathbf{x} \right> = r\left< \mathbf{a}, \mathbf{x} \right> = 0$, for each $\mathbf{a}, \mathbf{b}\in V^{\perp}$ and $r\in\mathbb{R}$.

\subsection*{Exercise 3.4}

Let $v\in V$. Then, since they generate $V$, we know that there exist $t_1,\dots,t_k\in\mathbb{R}$ so that $v=t_1a_1 + \dots t_ka_k$. But then $\left< x, v \right> = \left< x, t_1a_1 + \dots t_ka_k \right> = t_1\left< x, a_1 \right> + \dots + t_k\left< x, a_k \right>=0$, so that $x\in V^{\perp}$.

\subsection*{Exercise 3.5}
Observe that
$$\left< x+y, x+y\right> = \left< x, x\right> + \left< x, y\right> + \left< y, x\right> + \left< y, y\right> = \left< x, x\right> + 2\left< x, y\right> + \left< y, y\right>,$$
and
$$\left< x-y, x-y\right> = \left< x, x\right> - \left< x, y\right> - \left< y, x\right> + \left< y, y\right> = \left< x, x\right> - 2\left< x, y\right> + \left< y, y\right>,$$
so that
$$\left< x+y, x+y\right> - \left< x-y, x-y\right> = 4\left< x,y \right>$$
as desired.
...
$$
\left< x,y \right> = \left< \sum_{i} t_ia_i, y \right> = \sum_{i} t_i\left< a_i,y \right> = \sum_{i} t_i\left< a_i, \sum_{j} s_ja_j  \right> = \sum_{i} \sum_{j} t_is_j \left< a_i, a_j \right>.
$$

Since the $a_r$ form an orthonormal set, $\left< a_i, a_j \right>$ is non-zero (and in fact $1$) only when $i=j$, so that the above becomes

$$
\sum_{i} t_i s_i \left< a_i, a_i\right> = \sum_{i} t_i s_i.
$$
...
Then, according to the Gram-Schmidt orthogonalization process, set
\begin{align*}
    &w_1 \begin{aligned}[t] &= v_1\\
        &= \left(1,0,0,1\right),\end{aligned}\\ \\
    &w_2 \begin{aligned}[t] &= v_2 - \frac{\left< v_2, w_1 \right>}{\left< w_1, w_1 \right>}w_1\\
        &= \left(-1,0,2,1\right), \end{aligned}\\ \\
    &w_3 \begin{aligned}[t] &= v_3 - \frac{\left< v_3, w_1 \right>}{\left< w_1, w_1 \right>}w_1 - \frac{\left< v_3, w_2 \right>}{\left< w_2, w_2 \right>}w_2\\
        &=\left(\frac{2}{3}, 1, \frac{10}{3}, -\frac{2}{3}\right), \end{aligned}\\ \\
    &w_4 \begin{aligned}[t] &= v_4 - \frac{\left< v_4, w_1 \right>}{\left< w_1, w_1 \right>}w_1 - \frac{\left< v_4, w_2 \right>}{\left< w_2, w_2 \right>}w_2 - \frac{\left< v_4, w_3 \right>}{\left< w_3, w_3 \right>}w_3\\
        &=\left(-\frac{2}{5}, \frac{2}{5}, \frac{2}{3}, \frac{2}{5}\right).\end{aligned}
\end{align*}

This set of $w_i$ suffices.
...
\section*{Section 4: Linear Mappings and Matrices}

\subsection*{Exercise 4.1}

If the mapping $f:V\rightarrow W$ satisfies both $\left(2\right)$ and $\left(3\right)$, then
\begin{align*}
    f(a\mathbf{x} + b\mathbf{y}) &= f(a\mathbf{x}) + f(b\mathbf{y}) \text{ (additivity)}\\
    &= af(\mathbf{x}) + bf(\mathbf{y}) \text{ (homogeneity)},
\end{align*}
for each $\mathbf{x}, \mathbf{y} \in V$ and $a,b\in\mathbb{R}$, making it linear. Conversely, if $f$ is linear, then with $\mathbf{y}=\mathbf{0}$,
$$
f(a\mathbf{x}+b\mathbf{y}) = af(\mathbf{x}) + bf(\mathbf{y}) \implies f(a\mathbf{x}) = af(\mathbf{x}),
$$
making it homogeneous. Similarly, with $a=b=1$, $f$ is additive.
